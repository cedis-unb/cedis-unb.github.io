---
title: "Segurança da gov.br: dissertação avalia fatores adicionais de autenticação para a identidade digital"
date: 2025-02-18T00:00:00-03:00
draft: false
weight: 500
language: pt
featured_image: "../assets/images/featured/area_Cibersecurity.png"
summary: 'Sandro Leite Furtado defendeu no PPCA/UnB uma dissertação que identifica e avalia fatores adicionais de segurança para a Identidade gov.br, implementando e testando o RBA (Risk-Based Authentication) como camada complementar.'
author: CEDIS
authorimage: ../assets/images/global/author.webp
categories:
- News
tags:
- News
- defesa
- security
- digital_transformation
---
Brasília, 18 de fevereiro de 2025

A Identidade gov.br é a porta de entrada para dezenas de serviços públicos digitais brasileiros — e, por isso mesmo, o alvo predileto de agentes mal-intencionados dispostos a converter acesso em dano financeiro. **A dissertação defendida em fevereiro de 2025 por Sandro Leite Furtado no PPCA/UnB propõe elevar a segurança dessa porta com fatores adicionais de autenticação**, sob orientação do Prof. Dr. {{< link-interno "/people/george_marsicano" "George Marsicano Corrêa" >}}.

O trabalho combina revisão sistemática da literatura com estudo de caso aplicado à ID gov.br. Da revisão emergem cinco fatores promissores: RBA (Risk-Based Authentication), Tripwire, login rituals, OTP e OTP com Bluetooth. Dentre eles, o RBA foi implantado e avaliado em contexto real. Os resultados mostraram capacidade concreta de detectar comportamentos atípicos e bloquear tentativas suspeitas — especialmente em situações de divergência de localidade de acesso, um padrão clássico de conta comprometida.

A defesa aconteceu em 18 de fevereiro de 2025 em Brasília. A banca examinadora foi presidida pelo orientador, Prof. Dr. George Marsicano Corrêa (CIC/UnB), e composta pelo Prof. Dr. John Lenon Cardoso Gardenghi (FCTE/UnB) e pelo Prof. Dr. Milton Vinicius Morais de Lima (CISSA/CESAR). A Prof.ª Dr.ª Edna Dias Canedo, coordenadora do PPCA, também acompanhou a sessão.

Ao articular {{< link-interno "/areas/security/" "cibersegurança" >}} com {{< link-interno "/areas/transformation/" "transformação digital" >}} do Estado brasileiro, a dissertação oferece base concreta para decisões arquiteturais que afetam diretamente milhões de cidadãos — precisamente onde a comunidade de segurança precisa levar sua pesquisa para gerar impacto.

A agenda da dissertação também aparece no artigo [_Identification and Applicability of Additional Security Factors on the GOV.BR Digital Identity Platform_](https://doi.org/10.5748/19CONTECSI/PSE/GOV/7034), apresentado no 19th CONTECSI. O artigo consolida a discussão sobre fatores adicionais de autenticação aplicáveis à identidade digital brasileira.

O texto integral está disponível no [Repositório Institucional da UnB](https://repositorio.unb.br/handle/10482/52683).

---

Sobre o CEDIS:
O Centro de Estudos, Desenvolvimento e Inovação de Software (CEDIS), vinculado à Universidade de Brasília, pesquisa e desenvolve soluções inovadoras em software.
