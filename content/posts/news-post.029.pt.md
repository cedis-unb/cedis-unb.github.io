---
title: "INPI concede registro de software para o Chatbot de Engenharia de Requisitos"
date: 2026-08-25T09:00:00-03:00
draft: false
weight: 500
language: pt
featured_image: "../assets/images/featured/projeto_SuperR.png"
summary: "O software Chatbot para Engenharia de Requisitos recebeu certificado de registro de programa de computador concedido pelo INPI, referente ao processo BR512026003514-7."
description: "O registro do Chatbot para Engenharia de Requisitos formaliza a proteção jurídica do software junto ao INPI, no âmbito do projeto Super-R."
author: CEDIS
authorimage: ../assets/images/global/author.webp
categories:
- News
tags:
- News
- ai
- software_requirements
- chatbot_requisitos
- project_super_r
related_ids:
- chatbot_requisitos
- george_marsicano
- project_super_r
---

Brasília, 25 de agosto de 2026

O software **Chatbot para Engenharia de Requisitos** recebeu **certificado de registro de programa de computador** concedido pelo **Instituto Nacional da Propriedade Industrial (INPI)**, referente ao processo **BR512026003514-7**.

O produto foi desenvolvido por **A. C. M. Lustosa Junior** e **P. H. M. Oliveira**, sob orientação do Prof. {{< link-interno "/people/george_marsicano" "George Marsicano Corrêa" >}}, no âmbito do {{< link-interno "/projects/super_r" "projeto Super-R" >}}.

O software investiga como chatbots baseados em modelos de linguagem natural (LLMs) podem apoiar a Engenharia de Requisitos, conduzindo entrevistas estruturadas com stakeholders para elicitar, esclarecer e formalizar requisitos. Como resultado, gera automaticamente histórias de usuário (user stories), permite validação e correção durante a conversa e exporta o resultado em planilha (.xlsx).

Aplicável em organizações públicas e privadas que precisam levantar requisitos com múltiplos envolvidos, a solução resolve problemas recorrentes de comunicação falha, ambiguidade e baixa padronização na documentação de requisitos, reduzindo esforço manual e retrabalho. Diferencia-se de ferramentas baseadas em PLN tradicional por usar LLMs para adaptar perguntas ao contexto, detectar ambiguidades e gerar user stories de forma automatizada. Mais informações estão disponíveis na {{< link-interno "/products/chatbot_requisitos" "página do produto no CEDIS" >}}.

O registro no INPI assegura a proteção jurídica do programa de computador por 50 anos, conforme estabelece a **Lei nº 9.609/1998**, e amplia o portfólio protegido do CEDIS iniciado com o registro do {{< link-interno "/posts/news-post.023" "Contextus" >}}.

## Informações do registro

- Título do programa: Chatbot como Ferramenta de Apoio à Engenharia de Requisitos
- Processo: BR512026003514-7
- Ano do registro: 2026
- Publicador: Instituto Nacional da Propriedade Industrial (INPI)
