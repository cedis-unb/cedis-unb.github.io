---
title: "INPI concede registro de software para o app Pró-vida e Eu Me Protejo"
date: 2026-07-14T09:00:00-03:00
draft: false
weight: 500
language: pt
featured_image: "../assets/images/featured/projeto_Software_Bem.png"
summary: "O aplicativo Pró-vida e Eu Me Protejo recebeu certificado de registro de programa de computador concedido pelo INPI, referente ao processo BR512026005120-7."
description: "O registro formaliza a proteção jurídica do aplicativo mobile gamificado e offline voltado à prevenção da violência na infância."
author: CEDIS
authorimage: ../assets/images/global/author.webp
categories:
- News
tags:
- News
- gamification
- education
- social_software
- prevencao_violencia_infancia
- project_software_bem
related_ids:
- prevencao_violencia_infancia
- george_marsicano
- project_software_bem
---

Brasília, 14 de julho de 2026

O software **Pró-vida e Eu Me Protejo** — registrado no INPI sob o título oficial **"Aplicativo mobile gamificado para prevenção da violência na infância voltado a pais e educadores"** — recebeu **certificado de registro de programa de computador** concedido pelo **Instituto Nacional da Propriedade Industrial (INPI)**, referente ao processo **BR512026005120-7**, publicado na **Revista da Propriedade Industrial (RPI) nº 2897, de 14 de julho de 2026**.

O produto foi desenvolvido por **G. M. Rodrigues** e **S. A. C. Oliveira**, sob orientação do Prof. {{< link-interno "/people/george_marsicano" "George Marsicano Corrêa" >}}, associado ao {{< link-interno "/projects/software_bem" "programa Software para o Bem" >}}. A aplicação é derivada de conteúdos do projeto **"Pró-vida e Eu Me Protejo"** — parceria entre a instituição Pró-Vida e o projeto Eu Me Protejo — especialmente do **Manual de Boas Práticas**.

O aplicativo mobile gamificado (React Native + TypeScript, com build via EAS/APK) entrega o conteúdo do manual em formato interativo e orientado a progresso, com quizzes, XP, emblemas e cenários simulados, permitindo que o usuário avance de fase e receba recompensas à medida que aprende. Opera com acesso **offline-first**: após a instalação, funciona sem internet e preserva o progresso do usuário. Destina-se a pais, tutores e educadores, com foco em prevenção da violência na infância.

Diferentemente do consumo tradicional do manual em PDF, a experiência gamificada e offline aumenta engajamento e acessibilidade em contextos com conectividade limitada. Mais informações na {{< link-interno "/products/prevencao_violencia_infancia" "página do produto no CEDIS" >}}.

O registro no INPI assegura a proteção jurídica do programa de computador por 50 anos, conforme estabelece a **Lei nº 9.609/1998**.

## Informações do registro

- Título oficial do programa: Aplicativo mobile gamificado para prevenção da violência na infância voltado a pais e educadores
- Nome do projeto: Pró-vida e Eu Me Protejo
- Processo: BR512026005120-7
- Publicação: RPI nº 2897, de 14 de julho de 2026
- Publicador: Instituto Nacional da Propriedade Industrial (INPI)
