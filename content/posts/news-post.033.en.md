---
title: "INPI grants software registration for the Eu Me Protejo application"
date: 2026-08-25T09:20:00-03:00
draft: false
weight: 500
language: en
featured_image: "../assets/images/featured/projeto_Software_Bem.png"
summary: "The Eu Me Protejo application received a computer program registration certificate granted by INPI, under process BR512026005122-3."
description: "The registration formalizes the legal protection of the gamified mobile application for children ages 4 to 10, aligned with the Eu Me Protejo project."
author: CEDIS
authorimage: ../assets/images/global/author.webp
categories:
- News
tags:
- News
- gamification
- social_software
- prevencao_abuso_infantil
- project_software_bem
related_ids:
- prevencao_abuso_infantil
- george_marsicano
- project_software_bem
---

Brasilia, August 25, 2026

The **Eu Me Protejo Application** software received a **computer program registration certificate** granted by the **Brazilian National Institute of Industrial Property (INPI)**, under process **BR512026005122-3**.

The product was developed by **V. S. Cabral**, under the supervision of Prof. {{< link-interno "/people/george_marsicano" "George Marsicano Corrêa" >}}, associated with the {{< link-interno "/projects/software_bem" "Software for Good" >}} program. It was built in alignment with the educational materials of the **Eu Me Protejo** project, particularly its two main booklets.

The gamified mobile application, aimed at children ages **4 to 10** with use mediated by parents, guardians, or educators, supports learning and preventive education around self-protection and the prevention of child sexual abuse (CSA). The solution was built through action research with specialists, with requirements definition and development guided by gamification practices and plain language, staying aligned with materials validated by the project.

Unlike the traditional textual format of the booklets, the application offers engagement through gamification and playful elements to facilitate learning about a sensitive subject, with the potential for free use by schools and families — democratizing access to this preventive content. More information is available on the {{< link-interno "/products/prevencao_abuso_infantil" "product page at CEDIS" >}}.

The INPI registration secures legal protection of the computer program for 50 years, as established by **Law No. 9.609/1998**.

## Registration Information

- Program title: Eu Me Protejo Application
- Process: BR512026005122-3
- Registration year: 2026
- Publisher: Brazilian National Institute of Industrial Property (INPI)
