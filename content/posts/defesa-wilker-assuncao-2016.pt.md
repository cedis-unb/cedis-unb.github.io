---
title: "Algoritmos com pontos: TCC adapta ambiente virtual ao ensino gamificado de programação"
date: 2016-06-15T00:00:00-03:00
draft: false
weight: 500
language: pt
featured_image: "../assets/images/featured/area_Gamificação.png"
summary: 'Wilker Mesquita de Assunção defendeu na UnB Gama um TCC que adapta um ambiente virtual de aprendizado para o ensino de algoritmos usando técnicas de gamificação.'
author: CEDIS
authorimage: ../assets/images/global/author.webp
categories:
- News
tags:
- News
- defesa
- gamification
- education
---
Brasília, 2016

Ensinar algoritmos é ensinar rigor. O problema é que rigor, sem contrapeso, cansa; sem engajamento, o aluno solta a mão do problema no primeiro obstáculo. **É esse desequilíbrio que o TCC defendido por Wilker Mesquita de Assunção em 2016, na UnB Gama, tenta corrigir** — orientado pelo Prof. {{< link-interno "/people/sergio_freitas" "Sergio Antônio Andrade de Freitas" >}}.

O trabalho parte de uma escolha metodológica: em vez de propor um novo ambiente, adaptar um ambiente virtual de aprendizado já em uso, incorporando técnicas de gamificação como camada de motivação. A ideia é aproveitar a infraestrutura pedagógica existente e nela apoiar mecânicas que estimulam persistência — pontos, distintivos, missões — sem substituir o conteúdo pelo enfeite.

O tema costura {{< link-interno "/areas/gamification/" "gamificação" >}} com {{< link-interno "/areas/active_learning/" "aprendizagem ativa" >}} e antecipa, num contexto acadêmico, discussões que a comunidade ampliaria nos anos seguintes: como calibrar mecânicas de jogos para não escalar competição a ponto de esvaziar o objetivo pedagógico. O ambiente resultante é apresentado como prova de conceito para futuros experimentos em disciplinas introdutórias de programação.

O TCC pode ser lido na [Biblioteca Digital da Produção Intelectual Discente da UnB](https://www.bdm.unb.br/handle/10483/14870).

---

Sobre o CEDIS:
O Centro de Estudos, Desenvolvimento e Inovação de Software (CEDIS), vinculado à Universidade de Brasília, pesquisa e desenvolve soluções inovadoras em software, com forte atuação em {{< link-interno "/areas/gamification/" "gamificação" >}}.
