---
title: "INPI grants software registration for the Pró-vida e Eu Me Protejo app"
date: 2026-07-14T09:00:00-03:00
draft: false
weight: 500
language: en
featured_image: "../assets/images/featured/projeto_Software_Bem.png"
summary: "The Pró-vida e Eu Me Protejo application received a computer program registration certificate granted by INPI, under process BR512026005120-7."
description: "The registration formalizes the legal protection of the gamified offline mobile application focused on preventing childhood violence."
author: CEDIS
authorimage: ../assets/images/global/author.webp
categories:
- News
tags:
- News
- gamification
- education
- social_software
- prevencao_violencia_infancia
- project_software_bem
related_ids:
- prevencao_violencia_infancia
- george_marsicano
- project_software_bem
---

Brasilia, July 14, 2026

The **Pró-vida e Eu Me Protejo** software — registered at INPI under the official title **"Gamified mobile application for the prevention of childhood violence, aimed at parents and educators"** — received a **computer program registration certificate** granted by the **Brazilian National Institute of Industrial Property (INPI)** under process **BR512026005120-7**, published in the **Industrial Property Gazette (RPI) No. 2897, dated July 14, 2026**.

The product was developed by **G. M. Rodrigues** and **S. A. C. Oliveira**, under the supervision of Prof. {{< link-interno "/people/george_marsicano" "George Marsicano Corrêa" >}}, and is associated with the {{< link-interno "/projects/software_bem" "Software for Good" >}} program. The application is derived from content of the **"Pró-vida e Eu Me Protejo"** project — a partnership between the Pró-Vida institution and the Eu Me Protejo project — particularly from the **Good Practices Manual**.

The gamified mobile application (React Native + TypeScript, built via EAS/APK) delivers the manual's content in an interactive, progress-oriented format, with quizzes, XP, badges, and simulated scenarios, allowing the user to advance through stages and receive rewards as they learn. It operates with an **offline-first** approach: after installation, it works without internet and preserves the user's progress. It is intended for parents, guardians, and educators, focused on preventing violence in childhood.

Unlike the traditional PDF consumption of the manual, the gamified offline experience increases engagement and accessibility in contexts with limited connectivity. More information is available on the {{< link-interno "/products/prevencao_violencia_infancia" "product page at CEDIS" >}}.

The INPI registration secures legal protection of the computer program for 50 years, as established by **Law No. 9.609/1998**.

## Registration Information

- Official program title: Gamified mobile application for the prevention of childhood violence, aimed at parents and educators
- Project name: Pró-vida e Eu Me Protejo
- Process: BR512026005120-7
- Publication: RPI No. 2897, dated July 14, 2026
- Publisher: Brazilian National Institute of Industrial Property (INPI)
