---
title: Similarities Building a Network between Researchers based on the Curriculum
  Lattes Platform
date: '2018-01-01T00:00:00-03:00'
draft: false
language: en
translationKey: publication_2018_costa_oliveira_edgard_similarities_building_a_network_between_researchers_based_on_th
generated_by: scripts/build_publications.py
canonical_source: data/productions.yaml
id: publication_2018_costa_oliveira_edgard_similarities_building_a_network_between_researchers_based_on_th
publication_index: 145
publication_group: scientific
publication_type: conference
schema_type: ScholarlyArticle
year: 2018
authors:
- COSTA OLIVEIRA, EDGARD
- ALVES DE JESUS, DIONLAN
- FREITAS, S. A. A.
- DIAS CANEDO, EDNA
authors_structured:
- name: COSTA OLIVEIRA, EDGARD
  id: edgard_costa_oliveira
  url: /people/edgard_costa_oliveira
- name: ALVES DE JESUS, DIONLAN
  id: alves_de_jesus_dionlan
  url: /people/alves_de_jesus_dionlan
- name: FREITAS, S. A. A.
- name: DIAS CANEDO, EDNA
  id: edna_canedo
  url: /people/edna_canedo
tags:
- learning_analytics
advisors: []
doi_isbn: 10.5220/0006664102030214
source_title: 20th International Conference on Enterprise Information Systems
publisher: ''
location: Funchal
pages: ''
volume: ''
external_url: ''
spotify_podcast: ''
github_repo: ''
zenodo_record: ''
summary: Conference by COSTA OLIVEIRA, EDGARD, ALVES DE JESUS, DIONLAN, FREITAS, S.
  A. A., DIAS CANEDO, EDNA (2018).
bibtex: "@inproceedings{2018-costa-oliveira-edgard-similarities-building-a-network-between-researchers-based-on-th,\n\
  \  author = {COSTA OLIVEIRA, EDGARD and ALVES DE JESUS, DIONLAN and FREITAS, S.\
  \ A. A. and DIAS CANEDO, EDNA},\n  title = {Similarities Building a Network between\
  \ Researchers based on the Curriculum Lattes Platform},\n  year = {2018},\n  booktitle\
  \ = {20th International Conference on Enterprise Information Systems},\n  address\
  \ = {Funchal},\n  doi = {10.5220/0006664102030214}\n}"
aliases:
- /publications/2018-costa-oliveira-edgard-similarities-building-a-network-between-researchers-based-on-th/
---
