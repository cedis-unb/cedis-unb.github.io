---
title: 'Rethinking HCI Education: Gamification, Heuristics, and Project-Based Learning
  in Action'
date: '2025-01-01T00:00:00-03:00'
draft: false
language: en
translationKey: publication_2025_ana_julia_mendes_santos_repensando_a_educacao_em_ihc_gamificacao_heuristicas_e_aprend
id: publication_2025_ana_julia_mendes_santos_repensando_a_educacao_em_ihc_gamificacao_heuristicas_e_aprend
publication_index: 258
publication_group: scientific
publication_type: conference
schema_type: ScholarlyArticle
year: 2025
authors:
- Ana Júlia Mendes Santos
- Maria Clara Oleari de Araújo
- Júlia Rocha Fortunato
- Maurício Ferreira de Araújo
- Sergio Antônio Andrade de Freitas
authors_structured:
- name: Ana Júlia Mendes Santos
- name: Maria Clara Oleari de Araújo
- name: Júlia Rocha Fortunato
- name: Maurício Ferreira de Araújo
  id: mauricio_ferreira_de_araujo
- name: Sergio Antônio Andrade de Freitas
  id: sergio_freitas
  url: /people/sergio_freitas
tags:
- gamification
- ihc
- sergio_freitas
- project_jornada
advisors: []
doi_isbn: 10.1007/978-3-032-05008-3_55
source_title: Human-Computer Interaction – INTERACT 2025
publisher: Springer Berlin Heidelberg
location: ''
pages: 106-116
volume: '16111'
external_url: ''
spotify_podcast: ''
github_repo: ''
zenodo_record: ''
summary: This article proposes a pedagogical approach to teaching Human-Computer Interaction
  (HCI) in Software Engineering by combining heuristic evaluation, gamification, and
  project-based learning. Using questionnaires, statistical analysis, prototyping,
  and user validation, the method—based on the Octalysis framework—proved effective
  in improving interface quality and student engagement through educational (Role-Playing
  Game) RPG elements.
bibtex: "@inproceedings{2025-ana-julia-mendes-santos-repensando-a-educacao-em-ihc-gamificacao-heuristicas-e-aprend,\n\
  \  author = {Ana Júlia Mendes Santos and Maria Clara Oleari de Araújo and Júlia\
  \ Rocha Fortunato and Maurício Ferreira de Araújo and Sergio Antônio Andrade de\
  \ Freitas},\n  title = {Rethinking HCI Education: Gamification, Heuristics, and\
  \ Project-Based Learning in Action},\n  year = {2025},\n  volume = {16111},\n  pages\
  \ = {106-116},\n  publisher = {Springer Berlin Heidelberg},\n  doi = {10.1007/978-3-032-05008-3_55}\n\
  }"
aliases:
- /publications/2025-ana-julia-mendes-santos-repensando-a-educacao-em-ihc-gamificacao-heuristicas-e-aprend/
---
## Abstract

This article proposes a pedagogical approach to teaching Human-Computer Interaction (HCI) in Software Engineering by combining heuristic evaluation, gamification, and project-based learning. Using questionnaires, statistical analysis, prototyping, and user validation, the method—based on the Octalysis framework—proved effective in improving interface quality and student engagement through educational (Role-Playing Game) RPG elements.
