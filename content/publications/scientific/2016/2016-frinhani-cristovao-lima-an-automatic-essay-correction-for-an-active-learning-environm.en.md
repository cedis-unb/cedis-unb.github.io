---
title: An automatic essay correction for an active learning environment
date: '2016-01-01T00:00:00-03:00'
draft: false
language: en
translationKey: publication_2016_frinhani_cristovao_lima_an_automatic_essay_correction_for_an_active_learning_environm
id: publication_2016_frinhani_cristovao_lima_an_automatic_essay_correction_for_an_active_learning_environm
publication_index: 176
publication_group: scientific
publication_type: conference
schema_type: ScholarlyArticle
year: 2016
authors:
- FRINHANI, CRISTOVAO LIMA
- FREITAS, S. A. A.
- FERNANDES, MAURICIO VIDOTTI
- DIAS CANEDO, EDNA
authors_structured:
- name: FRINHANI, CRISTOVAO LIMA
  id: cristovao_de_lima_frinhani
- name: FREITAS, S. A. A.
- name: FERNANDES, MAURICIO VIDOTTI
- name: DIAS CANEDO, EDNA
  id: edna_canedo
  url: http://lattes.cnpq.br/9554285834432091
tags:
- ai
- machine_learning
- nlp
- sergio_freitas
advisors: []
doi_isbn: 10.1109/AICCSA.2016.7945769
source_title: 2016 IEEE/ACS 13th International Conference of Computer Systems and
  Applications (AICCSA)
publisher: ''
location: Agadir
pages: ''
volume: ''
external_url: ''
spotify_podcast: ''
github_repo: ''
zenodo_record: ''
summary: Conference by FRINHANI, CRISTOVAO LIMA, FREITAS, S. A. A., FERNANDES, MAURICIO
  VIDOTTI, DIAS CANEDO, EDNA (2016).
bibtex: "@inproceedings{2016-frinhani-cristovao-lima-an-automatic-essay-correction-for-an-active-learning-environm,\n\
  \  author = {FRINHANI, CRISTOVAO LIMA and FREITAS, S. A. A. and FERNANDES, MAURICIO\
  \ VIDOTTI and DIAS CANEDO, EDNA},\n  title = {An automatic essay correction for\
  \ an active learning environment},\n  year = {2016},\n  booktitle = {2016 IEEE/ACS\
  \ 13th International Conference of Computer Systems and Applications (AICCSA)},\n\
  \  address = {Agadir},\n  doi = {10.1109/AICCSA.2016.7945769}\n}"
aliases:
- /publications/2016-frinhani-cristovao-lima-an-automatic-essay-correction-for-an-active-learning-environm/
---
