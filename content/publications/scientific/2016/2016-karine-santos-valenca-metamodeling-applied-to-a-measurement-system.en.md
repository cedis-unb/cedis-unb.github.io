---
title: Metamodeling applied to a measurement system
date: '2016-01-01T00:00:00-03:00'
draft: false
language: en
translationKey: publication_2016_karine_santos_valenca_metamodeling_applied_to_a_measurement_system
generated_by: scripts/build_publications.py
canonical_source: data/productions.yaml
id: publication_2016_karine_santos_valenca_metamodeling_applied_to_a_measurement_system
publication_index: 322
publication_group: scientific
publication_type: conference
schema_type: ScholarlyArticle
year: 2016
authors:
- Karine Santos Valença
- Edna Dias Canedo
- Ricardo Ajax Dias Kosloski
authors_structured:
- name: Karine Santos Valença
  id: karine_santos_valenca
  url: /people/karine_santos_valenca
- name: Edna Dias Canedo
  id: edna_canedo
  url: /people/edna_canedo
- name: Ricardo Ajax Dias Kosloski
  id: ricardo_ajax
  url: /people/ricardo_ajax
tags:
- software_quality
advisors: []
doi_isbn: ''
source_title: 11th Iberian Conference on Information Systems and Technologies (CISTI)
publisher: ''
location: Gran Canaria, Spain
pages: '1'
volume: ''
external_url: ''
spotify_podcast: ''
github_repo: ''
zenodo_record: ''
summary: Conference by Karine Santos Valença, Edna Dias Canedo, Ricardo Ajax Dias
  Kosloski (2016).
bibtex: "@inproceedings{2016-karine-santos-valenca-metamodeling-applied-to-a-measurement-system,\n\
  \  author = {Karine Santos Valença and Edna Dias Canedo and Ricardo Ajax Dias Kosloski},\n\
  \  title = {Metamodeling applied to a measurement system},\n  year = {2016},\n \
  \ booktitle = {11th Iberian Conference on Information Systems and Technologies (CISTI)},\n\
  \  pages = {1},\n  address = {Gran Canaria, Spain}\n}"
aliases:
- /publications/2016-karine-santos-valenca-metamodeling-applied-to-a-measurement-system/
---
