---
title: Gamificação e avaliação do engajamento dos estudantes em uma disciplina técnica
  de curso de graduação
date: '2016-01-01T00:00:00-03:00'
draft: false
language: pt
translationKey: publication_2016_freitas_s_a_a_gamificacao_e_avaliacao_do_engajamento_dos_estudantes_em_uma_disciplina
generated_by: scripts/build_publications.py
canonical_source: data/productions.yaml
id: publication_2016_freitas_s_a_a_gamificacao_e_avaliacao_do_engajamento_dos_estudantes_em_uma_disciplina
publication_index: 172
publication_group: scientific
publication_type: conference
schema_type: ScholarlyArticle
year: 2016
authors:
- FREITAS, S. A. A.
- LIMA, THIAGO
- CANEDO, EDNA
- COSTA, RICARDO LOPES
authors_structured:
- name: FREITAS, S. A. A.
- name: LIMA, THIAGO
- name: CANEDO, EDNA
  id: edna_canedo
  url: /people/edna_canedo
- name: COSTA, RICARDO LOPES
  id: costa_ricardo_lopes
  url: /people/costa_ricardo_lopes
tags:
- gamification
- active learning
advisors: []
doi_isbn: 10.5753/cbie.sbie.2016.370
source_title: XXVII Simpósio Brasileiro de Informática na Educação
publisher: ''
location: Uberlandia
pages: 370-379
volume: ''
external_url: ''
spotify_podcast: ''
github_repo: ''
zenodo_record: ''
summary: Conference de FREITAS, S. A. A., LIMA, THIAGO, CANEDO, EDNA, COSTA, RICARDO
  LOPES (2016).
bibtex: "@inproceedings{2016-freitas-s-a-a-gamificacao-e-avaliacao-do-engajamento-dos-estudantes-em-uma-disciplina,\n\
  \  author = {FREITAS, S. A. A. and LIMA, THIAGO and CANEDO, EDNA and COSTA, RICARDO\
  \ LOPES},\n  title = {Gamificação e avaliação do engajamento dos estudantes em uma\
  \ disciplina técnica de curso de graduação},\n  year = {2016},\n  booktitle = {XXVII\
  \ Simpósio Brasileiro de Informática na Educação},\n  pages = {370-379},\n  address\
  \ = {Uberlandia},\n  doi = {10.5753/cbie.sbie.2016.370}\n}"
aliases:
- /publications/2016-freitas-s-a-a-gamificacao-e-avaliacao-do-engajamento-dos-estudantes-em-uma-disciplina/
---
