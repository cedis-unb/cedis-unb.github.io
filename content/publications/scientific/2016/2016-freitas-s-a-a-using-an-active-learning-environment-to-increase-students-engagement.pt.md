---
title: Using an Active Learning Environment to Increase Students' Engagement
date: '2016-01-01T00:00:00-03:00'
draft: false
language: pt
translationKey: publication_2016_freitas_s_a_a_using_an_active_learning_environment_to_increase_students_engagement
generated_by: scripts/build_publications.py
canonical_source: data/productions.yaml
id: publication_2016_freitas_s_a_a_using_an_active_learning_environment_to_increase_students_engagement
publication_index: 175
publication_group: scientific
publication_type: conference
schema_type: ScholarlyArticle
year: 2016
authors:
- FREITAS, S. A. A.
- SILVA, WANDER C. M. P.
- MARSICANO, GEORGE
authors_structured:
- name: FREITAS, S. A. A.
- name: SILVA, WANDER C. M. P.
  id: silva_m_c
  url: /people/silva_m_c
- name: MARSICANO, GEORGE
  id: george_marsicano
  url: /people/george_marsicano
tags:
- active_learning
advisors: []
doi_isbn: 10.1109/CSEET.2016.24
source_title: 2016 IEEE 29th International Conference on Software Engineering Education
  and Training (CSEET)
publisher: ''
location: Dallas
pages: ''
volume: ''
external_url: ''
spotify_podcast: ''
github_repo: ''
zenodo_record: ''
summary: Conference de FREITAS, S. A. A., SILVA, WANDER C. M. P., MARSICANO, GEORGE
  (2016).
bibtex: "@inproceedings{2016-freitas-s-a-a-using-an-active-learning-environment-to-increase-students-engagement,\n\
  \  author = {FREITAS, S. A. A. and SILVA, WANDER C. M. P. and MARSICANO, GEORGE},\n\
  \  title = {Using an Active Learning Environment to Increase Students' Engagement},\n\
  \  year = {2016},\n  booktitle = {2016 IEEE 29th International Conference on Software\
  \ Engineering Education and Training (CSEET)},\n  address = {Dallas},\n  doi = {10.1109/CSEET.2016.24}\n\
  }"
aliases:
- /publications/2016-freitas-s-a-a-using-an-active-learning-environment-to-increase-students-engagement/
---
