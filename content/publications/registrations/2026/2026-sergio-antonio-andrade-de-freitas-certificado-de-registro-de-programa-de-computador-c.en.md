---
title: 'Computer program registration certificate: Contextus'
date: '2026-01-01T00:00:00-03:00'
draft: false
language: en
translationKey: publication_2026_sergio_antonio_andrade_de_freitas_certificado_de_registro_de_programa_de_computador_c
generated_by: scripts/build_publications.py
canonical_source: data/productions.yaml
id: publication_2026_sergio_antonio_andrade_de_freitas_certificado_de_registro_de_programa_de_computador_c
publication_index: 0
publication_group: registrations
publication_type: registro
schema_type: SoftwareSourceCode
year: 2026
authors:
- Sergio Antônio Andrade de Freitas
- André Corrêa da Silva
- Milena de Faria Silva
- Renan Carneiro Batista
authors_structured:
- name: Sergio Antônio Andrade de Freitas
  id: sergio_freitas
  url: /people/sergio_freitas
- name: André Corrêa da Silva
  id: andre_silva
  url: /people/andre_silva
- name: Milena de Faria Silva
  id: milena_silva
  url: /people/milena_silva
- name: Renan Carneiro Batista
  id: renan_batista
  url: /people/renan_batista
tags:
- dfcris
- contextus
- ai
- machine_learning
advisors: []
doi_isbn: ''
source_title: ''
publisher: Instituto Nacional da Propriedade Industrial (INPI)
location: Brasília, DF, Brasil
pages: ''
volume: ''
external_url: /files/certificado_Contextus.pdf
spotify_podcast: ''
github_repo: ''
zenodo_record: ''
summary: Computer program registration granted by INPI under process BR512026000889-1,
  with the certificate issued on February 18, 2026.
bibtex: "@misc{2026-sergio-antonio-andrade-de-freitas-certificado-de-registro-de-programa-de-computador-c,\n\
  \  author = {Sergio Antônio Andrade de Freitas and André Corrêa da Silva and Milena\
  \ de Faria Silva and Renan Carneiro Batista},\n  title = {Computer program registration\
  \ certificate: Contextus},\n  year = {2026},\n  publisher = {Instituto Nacional\
  \ da Propriedade Industrial (INPI)},\n  address = {Brasília, DF, Brasil}\n}"
aliases:
- /publications/2026-sergio-antonio-andrade-de-freitas-certificado-de-registro-de-programa-de-computador-c/
---
## Abstract

Computer program registration granted by INPI under process BR512026000889-1, with the certificate issued on February 18, 2026.
