---
title: Otimização de índices de confiabilidade em redes de distribuição de energia
  elétrica
date: '2006-01-01T00:00:00-03:00'
draft: false
language: pt
translationKey: publication_2006_eduardo_zambon_otimizacao_de_indices_de_confiabilidade_em_redes_de_distribuicao_de_en
id: publication_2006_eduardo_zambon_otimizacao_de_indices_de_confiabilidade_em_redes_de_distribuicao_de_en
publication_index: 211
publication_group: thesis
publication_type: dissertation
schema_type: Thesis
year: 2006
authors:
- Eduardo Zambon
authors_structured:
- name: Eduardo Zambon
  id: eduardo_zambon
tags:
- software_architecture
advisors:
- berilhes_garcia
- sergio_freitas
doi_isbn: ''
source_title: ''
publisher: Biblioteca Central da Universidade Federal do Espirito Santo
location: ''
pages: ''
volume: ''
external_url: https://repositorio.ufes.br/handle/10/6352
spotify_podcast: ''
github_repo: ''
zenodo_record: ''
summary: Dissertation de Eduardo Zambon (2006).
bibtex: "@mastersthesis{2006-eduardo-zambon-otimizacao-de-indices-de-confiabilidade-em-redes-de-distribuicao-de-en,\n\
  \  author = {Eduardo Zambon},\n  title = {Otimização de índices de confiabilidade\
  \ em redes de distribuição de energia elétrica},\n  year = {2006},\n  publisher\
  \ = {Biblioteca Central da Universidade Federal do Espirito Santo}\n}"
aliases:
- /publications/2006-eduardo-zambon-otimizacao-de-indices-de-confiabilidade-em-redes-de-distribuicao-de-en/
---
